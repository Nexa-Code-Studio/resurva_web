...
import { Building2, CheckCircle, XCircle, Eye, X, MapPin, FileText, Users } from "lucide-react";

export default function EnterpriseVerificationPage() {
  const [verifications, setVerifications] = useState([
    { id: 1, name: "PT Boga Mandiri Group", director: "Bambang Pamungkas", location: "Jakarta Pusat", branchCount: 12, date: "2026-07-09", status: "Pending" },
    { id: 2, name: "CV Rasa Nusantara", director: "Anita Wijaya", location: "Bandung", branchCount: 5, date: "2026-07-11", status: "Pending" },
  ]);

  const [selectedEnterprise, setSelectedEnterprise] = useState<any>(null);

  const handleAction = (id: number, action: "Approve" | "Reject") => {
    if (confirm(`Apakah Anda yakin ingin ${action === "Approve" ? "menyetujui" : "menolak"} pendaftaran enterprise ini?`)) {
                ...
                {verifications.length === 0 ? (
                  ...
                  verifications.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">{item.director}</td>
                      <td className="px-6 py-4">{item.location}</td>
                      <td className="px-6 py-4 font-semibold text-resurva-dark">{item.branchCount} Cabang</td>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-base font-medium text-slate-900">{selectedEnterprise.director}</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase flex items-center gap-1"><Users className="w-3 h-3"/> Estimasi Cabang</p>
                    <p className="text-base font-medium text-slate-900">{selectedEnterprise.branchCount} Cabang Aktif</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-base font-medium text-slate-900">{selectedEnterprise.location}</p>
                    <p className="text-sm text-slate-500 mt-1">Gedung Perkantoran Sudirman Kav. 21, Lantai 5.</p>
                  </div>
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase">Tanggal Pengajuan</p>
                    <p className="text-base font-medium text-slate-900">{selectedEnterprise.date}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <div className="px-3 py-2 bg-white border border-slate-200 rounded-lg flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:bg-slate-50 transition-colors">
                    <FileText className="w-4 h-4 text-blue-500" /> Akta_Pendirian_PT.pdf
                  </div>
                  <div className="px-3 py-2 bg-white border border-slate-200 rounded-lg flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:bg-slate-50 transition-colors">
                    <FileText className="w-4 h-4 text-emerald-500" /> NIB_Perusahaan.pdf
                  </div>
                  <div className="px-3 py-2 bg-white border border-slate-200 rounded-lg flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:bg-slate-50 transition-colors">
                    <FileText className="w-4 h-4 text-rose-500" /> NPWP_Perusahaan.pdf
                  </div>
                </div>
              </div>
            </div>
